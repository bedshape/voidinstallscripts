...
read -p "enter mapper name for the root+boot+swap luks volume (default: rbs): " mapper_name
mapper_name=${mapper_name:-rbs}

# Prompt for block name of the rbs partition
read -p "enter block name of the rbs partition (e.g., /dev/sdaX): " rbs_block

# Prompt for block name of the EFI partition
read -p "enter block name of the EFI partition (e.g., /dev/sdaY): " efi_block
...
xchroot /mnt /bin/bash ./inchroot.sh || handle_error
